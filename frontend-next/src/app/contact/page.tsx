...
import type { ContactInfo, StoreCenter } from '@/lib/api/endpoints/content'
...
export default async function ContactPage() {
  ...
  const [contactInfo, storeCenters] = await Promise.all([
    contentApi.fetchContactInfo().catch(() => null),
    contentApi.fetchStoreCenters().catch(() => []),
  ])

  // Default fallback content for Contact Info (same as React version)
  ...
  const contactInfoContent = contactInfo || defaultContactInfo

  // Check if all opening hours are the same
  const allHoursSame =
    contactInfoContent.opening_hours_monday === contactInfoContent.opening_hours_tuesday &&
          ...
          <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="bg-white rounded-xl shadow-card p-6 sm:p-8">
              ...
              <div className="space-y-6">
                  ...
                  {allHoursSame ? (
                    ...
                    <div className="space-y-2">
                      {DAYS_OF_WEEK.map((day) => {
                        const hoursKey = `opening_hours_${day.key}` as keyof ContactInfo
                        const hours = contactInfoContent[hoursKey] as string
                        return (
                ...
                {/* Store Centers */}
                {storeCenters && storeCenters.length > 0 && (
                  <div className="pt-4 border-t border-beige-200">
