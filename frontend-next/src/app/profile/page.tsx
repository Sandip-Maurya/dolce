...
export default function ProfilePage() {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (formData.shippingAddress.zipCode && !/^[0-9]{6}$/.test(formData.shippingAddress.zipCode)) {
        ...
        <div className="max-w-2xl mx-auto mt-8">
          <div className="bg-white rounded-xl shadow-card p-6 sm:p-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <h2 className="text-xl font-heading text-charcoal-900 mb-4">Personal Information</h2>
                <div className="space-y-4">
                  <div>
                    <label htmlFor="name" className="block text-sm font-medium text-charcoal-700 mb-2">Full Name</label>
                    <input id="name" type="text" value={user.name} disabled className="w-full px-4 py-2 rounded-lg border border-beige-300 bg-beige-50 text-charcoal-600 cursor-not-allowed" />
                  ...
                  <div>
                    ...
                    <input id="email" type="email" value={profile?.email || user.email} disabled className="w-full px-4 py-2 rounded-lg border border-beige-300 bg-beige-50 text-charcoal-600 cursor-not-allowed" />
              ...
              <div className="border-t border-beige-200 pt-6">
                <h2 className="text-xl font-heading text-charcoal-900 mb-4">Shipping Address</h2>
                <div className="space-y-4">
                  <div>
                    <label htmlFor="street" className="block text-sm font-medium text-charcoal-700 mb-2">Street Address</label>
                    <input id="street" type="text" value={formData.shippingAddress.street} onChange={(e) => handleChange('shippingAddress.street', e.target.value)} className="w-full px-4 py-2 rounded-lg border border-beige-300 focus:outline-none focus:ring-2 focus:ring-charcoal-500" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="city" className="block text-sm font-medium text-charcoal-700 mb-2">City</label>
                      <input id="city" type="text" value={formData.shippingAddress.city} onChange={(e) => handleChange('shippingAddress.city', e.target.value)} className={`w-full px-4 py-2 rounded-lg border ${errors.city ? 'border-red-500' : 'border-beige-300'} focus:outline-none focus:ring-2 focus:ring-charcoal-500`} />
                      {errors.city && <p className="text-sm text-red-600 mt-1">{errors.city}</p>}
                    </div>
                    <div>
                      <label htmlFor="state" className="block text-sm font-medium text-charcoal-700 mb-2">State</label>
                      <input id="state" type="text" value={formData.shippingAddress.state} onChange={(e) => handleChange('shippingAddress.state', e.target.value)} className={`w-full px-4 py-2 rounded-lg border ${errors.state ? 'border-red-500' : 'border-beige-300'} focus:outline-none focus:ring-2 focus:ring-charcoal-500`} />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input id="zipCode" type="text" value={formData.shippingAddress.zipCode} onChange={(e) => handleChange('shippingAddress.zipCode', e.target.value.replace(/\D/g, '').slice(0, 6))} className={`w-full px-4 py-2 rounded-lg border ${errors.zipCode ? 'border-red-500' : 'border-beige-300'} focus:outline-none focus:ring-2 focus:ring-charcoal-500`} />
                    ...
                    <div>
                      ...
                      <input id="country" type="text" value={formData.shippingAddress.country} onChange={(e) => handleChange('shippingAddress.country', e.target.value)} className="w-full px-4 py-2 rounded-lg border border-beige-300 focus:outline-none focus:ring-2 focus:ring-charcoal-500" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-beige-200">
                <Button type="submit" variant="primary" isLoading={updateProfileMutation.isPending} disabled={updateProfileMutation.isPending}>
                  Save Changes
                </Button>
              </div>
            </form>
