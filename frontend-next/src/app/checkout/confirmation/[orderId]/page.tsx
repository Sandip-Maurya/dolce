      ...
      <div className="py-12">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl font-heading text-charcoal-900 mb-4">Thank You for Your Order!</h1>
          ...
          <div className="bg-white rounded-xl shadow-card p-6 sm:p-8 mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 pb-6 border-b border-beige-200">
              ...
              <div className="mt-4 sm:mt-0">
                <span className={`inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium ${statusColors[order.status] || statusColors.PLACED}`}>
            ...
            <div className="space-y-4 mb-6">
              ...
              {order.items.map((item) => (
                <div key={item.id} className="flex flex-col sm:flex-row gap-4 p-4 bg-beige-50 rounded-lg">
                  <div className="relative w-full sm:w-24 h-24 rounded-lg overflow-hidden bg-beige-100 flex-shrink-0">
                    <Image
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {item.product.tags.slice(0, 2).map((tag) => (
                        <Badge
                          ...
                          label={tag.name}
                          type={
                            tag.slug === 'organic' ? 'organic'
                              : tag.slug === 'eco-friendly' ? 'eco-friendly'
                              : tag.slug === 'sugar-free' ? 'sugar-free'
                              : tag.slug === 'artisan' ? 'artisan'
                              : 'custom'
                          }
                        />
                      ))}
                    </div>
                    <h4 className="text-lg font-heading text-charcoal-900 mb-1">{item.product.name}</h4>
                    <p className="text-sm text-charcoal-600 mb-2">Quantity: {item.quantity}</p>
                    <p className="text-lg font-heading text-charcoal-900">₹{item.line_total.toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-beige-50 rounded-xl p-6 sm:p-8 mb-8 text-center">
            <h3 className="text-xl font-heading text-charcoal-900 mb-4">Handcrafted with Care</h3>
            <p className="text-charcoal-700 leading-relaxed mb-4">
              Your order is being prepared by our team of skilled artisans who take pride in creating premium,
              sustainable treats. Every item is carefully handcrafted and packaged with eco-friendly materials.
            </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button variant="primary" className="w-full sm:w-auto" onClick={() => router.push('/orders')}>
              View All Orders
            </Button>
            <Button variant="secondary" className="w-full sm:w-auto" onClick={() => router.push('/products')}>
              Continue Shopping
            </Button>
          </div>
        </div>
